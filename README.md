...
![项目列表](./imgs/2-项目列表-1.png)

**项目列表-展开**

![项目列表-展开](./imgs/2-项目列表-2.png)
...
![新增项目资源](./imgs/4-新增项目资源.png)

**添加外部资源**

![添加外部资源](./imgs/5-添加外部资源.png)

**项目成员**

![项目成员](./imgs/6-项目成员.png)

**关联视图**

![关联视图](./imgs/7-关联视图.png)

**账号管理**

![账号管理](./imgs/8-账号管理.png)
...
- 当前认证采用 Bearer JWT。后端不维护服务端 session，前端在登录成功后会携带访问令牌请求 `/login/me` 恢复当前用户信息。
- 登录页“保持登录状态”勾选时，访问令牌会持久化到浏览器 `localStorage`；未勾选时仅保存在当前标签页的 `sessionStorage`，关闭该标签页后登录态失效。
- 受保护页面在未登录、令牌过期或收到 `401` 时会清理本地认证缓存并跳转到 `/login`；重新登录后会优先返回触发跳转前的目标页面。
- 相同登录令牌在多个标签页中使用时，退出登录会同步清理对应标签页中的该令牌，避免部分标签页残留失效登录态。
